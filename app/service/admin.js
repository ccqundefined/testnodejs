...
class AdminService extends Service {
  async checkAuth() {
    /*
           1.获取当前用户角色 (忽略权限判断的地址 is_super)
           2.根据角色获取当前角色的权限列表
           3.获取当前访问的url 对应的权限id
           4.判断当前访问的url对应的权限id是否在权限列表中的id中
       */
    // 3.获取当前访问的url 对应的权限id
    ...
    const ignoreUrl = [ '/admin/login', '/admin/doLogin', '/admin/verify', '/admin/loginOut', '/admin/changeStatus' ];
    ...
    const accessArray = []; // 获取当前角色可以访问的权限列表
  ...
  async getAuthList(role_id) {
    ...
    // console.log(this.ctx.session.userinfo.is_super)
